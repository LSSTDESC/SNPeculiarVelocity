...
%\sum_{\alpha}  \ldots \sum_{\delta}  \theta_{\alpha} \ldots \theta_\delta
%  \begin{pmatrix}
...
%& +   \frac{2}{\bar{N}}\int \sum_{i=1}^{N} n(x_1) (N(x_1)-\bar{N}) M_{gv}(x_1,x_i) \Delta v_i  d^3x_1 \\
%& + \sum_{i=1}^{N}  \sum_{j=1}^{N}  \Delta v_i M_{vv}(x_i,x_j) \Delta v_j\\
...
%\Delta \textbf{y}^T \textbf{C}^{-1} \textbf{S}_\beta \textbf{C}^{-1} \Delta \textbf{y} & \approx \sum_{i,j\in N_{gal}} N_i N_j \xi_{\delta\delta,\theta_\beta}(x_i,x_j) -2 \bar{N} \sum_{i\in N_{gal},j\in N_{cell}} \xi_{\delta\delta,\theta_\beta,ij}\\
...
First, we focus on the  term $\Delta \textbf{y}^T\textbf{C}^{-1}\Delta \textbf{y}$. We rewrite $\textbf{C}$ by normalizing out its uncorrelated $\textbf{C}_0$ term
by   introducing the matrices $\textbf{M}$ and $\Delta \textbf{y}$,
where $\textbf{M}=\sqrt{\textbf{C}_0}$ and $\textbf{S} = \textbf{M} \Delta \textbf{y} \textbf{M}$ such that $\Delta \textbf{y}=\textbf{M}^{-1}\textbf{S}\textbf{M}^{-1}$. Now, our matrix $\textbf{C}$ can be re-written in terms of $\Delta \textbf{y}$ and $\textbf{M}$:
\begin{equation}
\textbf{C} = \textbf{M}(\textbf{I}+A\Delta \textbf{y})\textbf{M}.
\end{equation}
Expanded as a Taylor series
\begin{align}
\textbf{C}^{-1} & = \textbf{M}^{-1}(1-A\Delta \textbf{y}+A^2 \Delta \textbf{y}^2 + \ldots) \textbf{M}^{-1} \\
...
\begin{align}
\Delta \textbf{y}^T \textbf{C}_0^{-1}\textbf{S}\textbf{C}_0^{-1} \Delta \textbf{y}& = (\textbf{N}-\bar{N}) \xi_{\delta\delta}  (\textbf{N}-\bar{N})  + 2  (\textbf{N}-\bar{N}) \xi_{\delta v}\frac{\mathbf{\Delta v}}{\bm{\sigma}_v^2}+ 
...
\begin{align*}
\Delta \textbf{y}^T \textbf{C}_0^{-1}\textbf{S}\textbf{C}_0^{-1} \Delta \textbf{y}& = \sum_{i,j\in N_{gal}} \xi_{\delta\delta,ij} -2 \bar{N} \sum_{i\in N_{gal},j\in N_{cell}} \xi_{\delta\delta,ij}\\
...
We evaluate $\Delta \textbf{y}^T \textbf{C}^{-1}\Delta \textbf{y}$ by splitting up the counts and velocities.  The data vector $\Delta \textbf{y}$ is composed of the counts
...
Then
\begin{align}
\Delta \textbf{y}^T \textbf{C}_0^{-1}\textbf{S}\textbf{C}_0^{-1}\textbf{S}\textbf{C}_0^{-1} \Delta \textbf{y}
...
We have assembled the contributions of $\Delta \textbf{y}^T \textbf{C}^{-1}\Delta \textbf{y}$ to second order in $A$.

Returning to the other contribution to the log-likelihood in Eq.~\ref{log:eqn} we have
\begin{align*}
\ln\det(\textbf{C}) &=\ln\det(\textbf{M}(\textbf{I}+A\Delta \textbf{y})\textbf{M})) \\
&= \ln\det(\textbf{I}+A\Delta \textbf{y}) +\text{const} \\
&= A \tr \Delta \textbf{y} -\frac{A^2}{2}\tr \Delta \textbf{y}^2+ O(A^3) + \ldots\\
\end{align*}
Now
\begin{equation}
\Delta \textbf{y} = \textbf{M}^{-1}\textbf{S}\textbf{M}^{-1} = 
\begin{pmatrix}
...
The trace of $\Delta \textbf{y}^2$ has terms, to leading order in $\bar{N}$, has terms
\begin{equation}
...
First, we focus on the second term, $\Delta \textbf{y}^T\textbf{C}^{-1}\Delta \textbf{y}$. Looking at the definition of $\textbf{C}$ in equation (1), we want to factor $\textbf{C}_0$ into an identity matrix. To do so, we introduce matrix $\textbf{M}$ and $\Delta \textbf{y}$. We let $\textbf{M}=\sqrt{\textbf{C}_0}$, and $\textbf{S} = \textbf{M} \Delta \textbf{y} \textbf{M}$. Then $\Delta \textbf{y}=\textbf{M}^{-1}\textbf{S}\textbf{M}^{-1}$. Now, our matrix $\textbf{C}$ can be re-written in terms of $\Delta \textbf{y}$ and $\textbf{M}$:
\begin{equation}
\textbf{C} = \textbf{M}(\textbf{I}+A\Delta \textbf{y})\textbf{M},
\end{equation}
and its inverse, to first order, is
\[
\textbf{C}^{-1} = \textbf{M}^{-1}(1-A\Delta \textbf{y}) \textbf{M}^{-1} = -A \textbf{C}_0^{-1}\textbf{S}\textbf{C}_0^{-1} + \text{const}.
\] \par
Having this expression, we can now compute the second term in equation (2). Denoting the value of this term by $s$, we have
\begin{align*}
s &=\Delta \textbf{y}^T \textbf{C}^{-1}\Delta \textbf{y} \\
&= -A\left(\Delta \textbf{y}^T \textbf{C}_0^{-1}\right)\textbf{S}\left(\textbf{C}_0^{-1}\Delta \textbf{y}\right).
\end{align*} \par
...
\[
-\left(\Delta \textbf{y}^T \textbf{C}_0^{-1}\right)\textbf{S}\left(\textbf{C}_0^{-1}\Delta \textbf{y}\right) = s_1 +s_2+\hdots +s_6
\]
\begin{align*}
