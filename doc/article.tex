...
The  peculiar velocity signal  can be described by its power spectrum $P_{vv}$.
...
at $z=0$ expected from General Relativity and linear theory. Available $k$ ranges for surveys with different
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{src/power.png}
%\includegraphics[width=0.5\textwidth]{../outcosmo/zmax_.png}
\caption{Top panel: Volume-weighted velocity power spectrum $k^3P_{vv}$ at $z=0$ for $\mu=1, 0,5$ as predicted for General Relativity in the linear regime.
Lower two panels: $I/I_{max}$ for $\mu=1, 0.5$  where $I=\text{Tr}\left(C^{-1}C_{,f\sigma_8}C^{-1}C_{,f\sigma_8}\right)$ represents the contribution of a single mode to
...
The performance of a peculiar velocity survey depends on its overlap with galaxy surveys, solid-angle coverage, redshift depth, and intrinsic magnitude dispersion.
...
While the expected variance in $f\sigma_8$ is $\left(F^{-1}\right)_{{f\sigma_{8}\,f\sigma_{8}}}$, we use $I$ as a proxy to help understand
how survey densities and shot noise affect science performance.
Surveys are compared relative to the $I_{max}$ of an idealized sample-variance limited peculiar-velocity survey with\ $\sigma/n =0$.
...
Consider two- and ten-year
SN~Ia densities $n$  based on SN~Ia rates 
with 0.65 efficiency, and velocity dispersions $\sigma \approx (\frac{5}{\ln{10}} \frac{1+z}{z})^{-1} \sigma_M$
