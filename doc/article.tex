...
Peculiar velocities are sensitive to the growth of structure through the combination $fD$, where $D$ is  the linear growth factor and
$f \equiv \frac{d\ln{D}}{d\ln{a}}$ is the linear growth rate.  The growth of structure depends on gravity, indeed 
...
For reference, t $z>0.3$ redshifts DESI projects 10\% precision of $fD$ using RSD alone.
...
hundreds of thousands, over the course of 10-years, LSST will find $\sim150,000$ $z<0.2$,  $\sim520,000$ $z<0.3$ SNe~Ia for which good light curves can be measured. This is a sample size with more galaxies at deeper redshifts than projected by WALLABY and TAIPAN.  


A 10-year LSST SN survey can produce interesting $fD$ measurements in redshift bins, as shown in Table~1 of 
\citet{2017ApJ...847..128H}.
Projected percent uncertainties in $f\sigma_8$ from 
...
Our scientific interest is in probing gravity.
...
As a probe of gravity, peculiar velocity surveys are sensitive to $fD=\Omega_M^{\gamma} a \exp{\left(\left(\int_0^a \Omega_M^{\gamma} -1\right)  d\ln(a)\right)}$.
Note that $\frac{d(\ln{fD})}{d\gamma} = \ln\Omega + \int \Omega^{\gamma} \ln{\Omega} d\ln{a} \approx -1.68, -0.75,-0.37$ at $z=0,0.5,1.0$ in $\Lambda$CDM,
meaning that surveys at low redshift better measure $\gamma$ given a fixed precent uncertainty in $fD$.
Non-GR models may  also predict a change in the scale-dependence of the growth, such observations provide additional leverage in probing gravity
but are not considered here.


The uncertainties in $\gamma$ for  RSD and peculiar velocity surveys are estimated using a
Fisher matrix analysis
similar to but with lower fidelity to that of \citet{2017ApJ...847..128H, 2017MNRAS.464.2517H}.
The analysis is performed in Fourier space, 
where the Fisher information matrix of parameters $\lambda \in \{\gamma,b\}$ from random Gaussian field with mean zero is given by
\begin{align}
