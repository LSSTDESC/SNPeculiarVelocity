...
\begin{abstract}
Cadenced wide-field imaging surveys 
...
SNe~Ia in the local $z<0.3$ Universe will provide a measure of $\gamma$ 
down to $0.01$ precision that can definitively distinguish  between General Relativity and leading models of alternative gravity.
\end{abstract}
...
and the line of sight.  While the $bD$ term does contain information on $\gamma$, it is not used for making constraints.
...
An analysis in which the RSD and velocity surveys are treated independently is achieved by
...
All the surveys considered can provide meaning tests of gravity.
The projected uncertainty in $\gamma$ achieved by the suite of surveys are shown in Figure~\ref{var:fig}.
For each survey uncertainties are based on three types of analyses, one that only uses peculiar velocities, one that uses
both overdensities (RSD) and peculiar velocities independently, and one  that uses
both RSD, peculiar velocities, and their cross-correlation.
...
The figure also reveals an interesting interplay between RSD and velocity measurements.  Peculiar velocities alone
can account for much of the probative power of the surveys. RSD alone do not provide significant constraints.
However, considering RSD and velocity cross-correlations decreases $\sigma_\gamma$ by  factors of $\sim 0.8$.
There are important
$k$-modes that are  sample variance limited either in  overdensity and/or peculiar velocity who benefit from the
noise suppression engendered by cross-correlations.

\begin{figure}
\centering
\includegraphics[width=0.47\textwidth]{src/var.png}
%\includegraphics[width=0.5\textwidth]{../outcosmo/zmax_.png}
\caption{The projected uncertainty in $\gamma$ achieved by two- (red) and ten-year (black) surveys of varying depth $z_{max}$.
...
both overdensities (RSD) and peculiar velocities independently (dotted), and one  that uses
...
This is demonstrated to be the case at the end of both 2- and 10-year surveys and $z_{max}=0.2$.
...
Increasing the survey duration  increases the number density of discovered supernovae. As
shot-noise is relevant for all the surveys considered, the increased survey duration improves the
precision in $\gamma$. The surveys that benefit most are those more susceptible to shot noise:
higher redshifts  due to their higher velocity dispersion and shorter surveys  due to their lower number 
densities.
...
This is evident in the plot of $\sigma_\gamma^{-1}\partial \sigma_\gamma/\partial \sigma_M$ shown in the bottom-right panel of Figure~\ref{dvar:fig}.
 \item[Minimum length scale, maximum wavenumber $k_{max}$]  There is a minimum length scale at which density and velocity distributions are reliably predicted from theory and
 are Gaussian-distributed.  Changes in this scale engender changes in the $\gamma$ precision as the survey-independent
  $\sigma_\gamma^{-1}\partial \sigma_\gamma/\partial k_{max} = 0.0050$ at $k_{max}=0.1h$\,Mpc$^{-1}$.
 \end{description}

From the above calculations, we find empirically that shot noise is important
but that sample variance is non-negligible at the end of longer surveys or when the intrinsic magnitude dispersion is small.
Fine tuning
...
We now touch on how survey choices affect $\Omega$, $z_{max}$, and $\sigma_M$. 
\begin{itemize}
...
(e.g.\ SNIFS, SED Machine; the surface density of active transients is low making MOS unnecessary).
...
an active subject of  research.  Note that photometric classification benefits from having spectroscopic redshifts.
...
bright-galaxy moderate-resolution redshift surveys (e.g.\ DESI, 4MOST) can also provide multiplexed observations of 
a significant fraction of nearby SN hosts before or well after SN discovery.
...
both, though there is a question whether the survey light curves are sufficient to derive precision distances.  For example, 
...
The imaging survey has a trade between  the solid angle ($\Omega$) it monitors and  the light-light quality ($\sigma_M$) it generates, whether it should serve
as a SN~Ia discovery and distance machine, or a discovery machine only and relying on a complementary suite of instruments to get distances.
...
For example, infrared data \citep{2012MNRAS.425.1007B} or spectrophotometry at peak brightness
 ...
 \item Telescope aperture--Exposure Time:  Light-collection capabilities depend on the fluxes of the targets.
 Closer and hence brighter objects require more modest telescope apertures and/or exposure times.  For our science case,
 low-redshift sources are most valuable.  $z_{max}$ occurs when complete target follow-up saturates resources.
 \end{itemize}

We note that a low-redshift supernova survey is insensitive to two important uncertainties
...
To give an idea of scale, a follow-up survey with $z_{max}=0.2$ would have $\sim 150,000$ targets over 10 years.  These sources will be observed when
brighter than $r \sim 20.5$~mag.  
