...
The expected peculiar velocity signal  is described by its power spectrum $P_{vv}$.
The range $k$'s  used in an analysis depends on the depth of the survey $k_{min} \approx (2 z_{max})^{-1}$, and the smallest
scales where the theoretical predictions are reliable, say $k_{max} \approx 0.1$~$h$\,Mpc$^{-1}$.
The top panel of Figure~\ref{detC:fig} shows the volume-weighted power spectrum $k^3 P_{vv}$
at $z=0$ assuming General Relativity and linear theory.  The panel shows available $k$ ranges for several surveys with different
maximum redshift: the $0.02 <k< 0.1$~$h$\,Mpc$^{-1}$ modes contribute the bulk of the peculiar velocity signal.
The sensitivity to gravity primarily resides in the normalization of the power spectrum, as  $P_{vv} \propto (f\sigma_8)^2$.
Non-GR models can also predict differences in the shape and the redshift-dependence of GR prediction $P_{vv}$  in Figure~\ref{power:fig}.

\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{src/power.png}
...
\citet{2017ApJ...847..128H} project how complementary galaxy and peculiar velocity surveys in a common volume can measure $f\sigma_8$. 
They perform their analysis in Fourier space, 
where the Fisher information matrix of parameters $\lambda_i$ from random Gaussian field with mean zero and covariance $C$ is
\begin{equation}
...
\propto  (b\sigma_8 + f\sigma_8\mu^2)f\sigma_8$, where $b$ is the bias parameter.
The galaxy number density is $n_g$, the number density of velocity probes is $n$, and the probe velocity dispersion
is $\sigma$. 
 The Fisher matrix in  Eq.~\ref{fisher:eqn} is evaluated by \citet{2017MNRAS.464.2517H}  to generate the $f \sigma_8$ uncertainties in Table~\ref{tab:howlett}.
...
\end{equation}
in Eq.~\ref{fisher:eqn}  represents how much a differential Fourier
volume element with wavenumber $k$ and line of sight $\mu$ contributes to $F_{f\sigma_{8}\,f\sigma_{8}}$;
it is therefore  useful in evaluating the design of peculiar velocity surveys. 
We evaluate these surveys relative to the $I_{max}$ of a sample-variance limited
peculiar velocity survey with\ $\sigma/n =0$.
In the following we consider as given a  TAIPAN-like
galaxy survey with $b=1.2$ and number density $n_g=10^{-3}h^3$\,Mpc$^{-3}$ that overlaps with the peculiar velocity surveys.

First, we show the benefit of having the  galaxy and velocity surveys in a common volume by considering the degradation that occurs when 
they are treated as being
independent.  For this exercise the velocity survey is taken to be sample-variance limited.
The resulting $I/I_{max}$  is shown as the black dashed curves in the bottom two panels of Figure~\ref{power:fig},
and is found to lie between $0.1-0.2$.
Taking advantage of the fact that the same dark-matter fluctuations produce the galaxies and velocities 
in the common volume decreases
the variance in $f\sigma_8$ by factors of 5 to 10.

Realistic surveys have shot noise. We consider two- and ten-year
supernova densities $n$  based on SN~Ia rates 
with 0.65 sample efficiency, and velocity dispersions $\sigma \approx (\frac{5}{\ln{10}} \frac{1+z}{z})^{-1} \sigma_M$
assuming a supernova magnitude dispersion of $\sigma_M=0.08$ and inserting values of $z=0.05,0.1, 0.2$.
The $I/I_{max}$ of these survey scenarios are shown in the bottom two panels of Figure~\ref{power:fig}.
Generally, $k<0.01$ modes have $I/I_{max}\sim 1$ meaning that they are sample-noise limited.
Individual $k>0.01$ modes have  $I/I_{max}<1$ showing that shot noise
is significant.  Recall that collectively,  larger $k$ modes are the important contributors to the peculiar-velocity signal;
