...
velocity power spectrum has dependence $P_{vv}\propto (HfD)^2$.  A supernova  survey  measures luminosity distance fluctuations
...
at low redshift.   The $H$-dependence of  $P_{vv}$ and the conversion from distances to velocities cancel, making  peculiar velocity surveys sensitive to $(fD)^2$.
}
...
surveys are  sensitive to $fD=\Omega_M^{\gamma} \exp{\left(\int_a^1 \Omega_M^{\gamma} d\ln{a} \right)}$, whose $\Lambda$CDM prediction is plotted 
...
\caption{Volume-weighted velocity power spectrum $k^3P_{vv}(z=0)$ for $\mu=1, 0.5$ (solid black, cyan)  as predicted for General Relativity in the linear regime.
Volume-weighted peculiar velocity shot-noise  $k^3\sigma^2/n$ at $z=0.1$, expected from 2- and 10-year (red, black) supernova densities, 0.08 and 0.15~mag (dashed, dotted) intrinsic magnitude dispersions.
...
hundreds of thousands, over the course of 10-years, LSST will find $\sim150,000$ $z<0.2$,  $\sim520,000$ $z<0.3$ SNe~Ia
 ...
 (the total rest-frame volumetric SN~Ia rate being $7.84\times 10^{-5} h^3$\,Mpc$^{-3}$\,yr$^{-1}$.)
  This sample has comparable
 number density and more galaxies at deeper redshifts than projected by WALLABY and TAIPAN. 

Given these two advances, over the course of a decade an LSST SN survey can produce interesting $fD$ measurements in several
redshift bins.   
...
$fD$, we now focus on $\gamma$.  Survey design should be tailored toward minimizing the uncertainty in $\gamma$
...
where the Fisher information matrix of parameters $\lambda \in \{\gamma,bD\}$ is
\begin{align}
F_{ij} 
...
\label{cov:eq}
\end{equation}
The parameter dependence enters through $fD$ in the relations $P_{vv}\propto (fD\mu)^2$, the SN~Ia overdensity
...
and the line of sight.  Note that while the $bD$ term does contain information on $\gamma$, we do not use it in this analysis.
...
The survey parameters that enter the calculation are the solid angle $\Omega$, depth of the survey $r_{max}$ (or $z_{max}$), the intrinsic SN~Ia magnitude dispersion 
...
An analysis in which the RSD and velocity surveys are treated independently is considered by
setting the off-diagonal elements of $C$ to zero.


We calculate  $\sigma_\gamma$ of  peculiar velocity surveys for a range of redshift depths $z_{max}$, with survey durations of $t=2$ and 10~years, covering $3\pi$  of sky, and with 
analyses accounting for or neglecting
cross-correlations.
The other survey parameters
$\Omega=3$, $\epsilon=0.65$, $\sigma_M=0.08$~mag are fixed.
The bias of SN~Ia hosts is taken to be $b=1.2$.
The uncertainties in $\gamma$ are shown in Figure~\ref{var:fig}.

\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{src/var.png}
%\includegraphics[width=0.5\textwidth]{../outcosmo/zmax_.png}
\caption{Projected $\sigma_\gamma$ of  peculiar velocity surveys for a range of redshift depths $z_{max}$, with survey durations of $t=2$ and 10~years, and with 
analyses accounting for or neglecting
cross-correlations.
\label{var:fig}}
\end{figure}

Shallow, short-duration peculiar-velocity surveys already achieve meaningful constraints on gravity.   A 2-year, $z_{max}=0.1$ survey has $\sigma_\gamma \sim 0.038$,
which can distinguish  between  General Relativity, $f(R)$,  and DGP gravities at the $2\sigma$ level.
Using cross-correlation information decreases $\sigma_\gamma$ by  factors of $\sim 0.8$.   
 This modest improvement 
comes from the suppression of  sample variance.
It is the density measurement that is sample-variance
limited,  the peculiar velocity measurement is shot-noise limited. 

Having established that peculiar velocity surveys can produce a precise test of gravity, we now consider survey optimization through the 
survey parameters  $\Omega$, $z_{max}$, $t$, and $\sigma_M$, and also show the relative importance of different
redshifts with a given survey.  An important survey parameter that is not considered is cost.
\begin{description}
\item[Solid Angle $\Omega$]
The Fisher Matrix $F$ is  proportional to the survey solid angle $\Omega$.  The baseline
...
\item[Differential Redshift Bin $z$] Certain differential redshift bins  constrain $\gamma$ more strongly than others.  
The left panel
of Figure~\ref{dvar:fig} shows $\partial \sigma_\gamma / \partial z$  for two- and ten-year surveys with limiting $z_{max}=0.2$.
In both cases the shape of the curve is the same, with the amplitude monotonically decreasing from $z=0.01$ out to $z=0.2$.  If we had
to sculpt the distribution (say due to limited follow-up resources), the preference would be to sculpt out the highest redshift bins
resulting in a decreased $z_{max}$.   The optimal redshift distribution is thus the SN-discovery distribution and the sample is unambiguously
parameterized by $z_{max}$.  Note that this result is based on constraining a constant redshift-independent $\gamma$.
\begin{figure}
\centering
\includegraphics[width=0.47\textwidth]{src/dvardz.png}
\includegraphics[width=0.47\textwidth]{src/dvardlnt.png}
%\includegraphics[width=0.5\textwidth]{../outcosmo/zmax_.png}
\caption{Left: $\partial \sigma_\gamma / \partial z$  for two- and ten-year surveys with limiting $z_{max}=0.2$.  Right: $\partial \sigma_\gamma / \partial \ln{t}$ as a function of
 $z_{max}$ for two- and ten-year surveys.
\label{dvar:fig}}
\end{figure}
\item[Redshift Depth $z_{max}$]
Increasing the survey redshift depth increases the $\gamma$ precision though that precision asymptotes
$z_{max} \sim 0.2$ as seen in Figure~\ref{var:fig}.
 \item[Survey duration $t$] There is a significant decrease on $\sigma_\gamma$ when going from a 2- to ten-year survey, as shown
 in  Figure~\ref{var:fig}.   The right panel of Figure~\ref{dvar:fig} shows $\partial \sigma_\gamma / \partial \ln{t}$ as a function of
 $z_{max}$ for two- and ten-year surveys.  The non-zero amplitudes show that shot-noise affects both two- and ten-year surveys, but
 much less so for the latter more so at low redshifts.  After 10-years, the effect of sample-variance important for shallower surveys.
\item[Intrinsic Magnitude Dispersion $\sigma_M$]
The relationship between intrinsic magnitude dispersion and precision in $\gamma$ can be seen through the 
previous result, as $\partial \sigma_\gamma/\partial \sigma_M = \partial \sigma_\gamma / \partial \ln{t}$.
...
Shot-noise limit touched upon several times in the preceding discussion.  Go back to Figure.

There are a number of interesting features to note:
\begin{itemize}
\item Ten-year survey has lower  $\sigma_\gamma$  relative to  two-year surveys with factors that are redshift-dependent.  For $z_{max}=0.3$
the uncertainties are reduced by a factor $\sim 0.5$ whereas at  $z_{max}=0.7$ the factor is $\sim 0.7$
\end{itemize}



 Shot noise
is important  Figure~\ref{power:fig}.
