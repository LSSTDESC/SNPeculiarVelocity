...
#Chao: not sure whwat this statement doe
#print("{} & {} & {} & {} & {} & {} & {} & {} \\\\".format("$z_{max}$", "fraction", "$\\sigma_{SN}$", "$N_{gal}$", \
#    "$\\bar{A}$", "$\\sigma_A$", "$\\sigma_A/\\sqrt{N_{gal}}$", "$\\bar{A}/\sigma_A\\sqrt{12000/4000}$"))

#Chao: commenting out the for loop
#for name,nsn,zma,fra,snsi in zip(names,nsne,zmax,frac,snsig):

name = "pvlist.0.08.1234.1.0.2.pkl.1000"
chain=  pickle.load(open("/project/projectdirs/m1727/akim/pvoutcosmo/debug/"+name,'rb'))
cutchain = chain[:,500:,:]
#print(cutchain)
     # plotter(cutchain)
c = ChainConsumer()
...
dum[:,3] = dum[:,3]*3e5
#c.add_chain(dum, parameters=["$A$", "$M$","$\sigma_M$","$\sigma_{v}$"])
c.add_chain(dum, parameters=["A", "M","sigma_M","sigma_v"])
c.plotter.plot(filename="/project/projectdirs/m1727/akim/pvoutcosmo/debug/"+name+".png", figsize="column",truth=[None,0,0.08,None])
